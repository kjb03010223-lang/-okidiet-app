...
// ── Cute chibi sea creatures matching reference image ─────────────────────────
// Style: round body, thick simple outline, solid fill, tiny black dot eyes only
...
// ── Sea creatures layer component ─────────────────────────────────────────────
function SeaCreatures() {
  return (
    <>
      {/* Whale crosses L→R */}
      <div style={{position:"fixed",top:"13%",pointerEvents:"none",zIndex:0,animation:"crossLR 28s linear 0s infinite"}}>
        <ChibiWhale/>
      </div>
      {/* Dolphin waves R→L */}
      <div style={{position:"fixed",top:"24%",pointerEvents:"none",zIndex:0,animation:"waveRL 19s linear 4s infinite"}}>
        <ChibiDolphin/>
      </div>
      {/* Jellyfish floats top-right */}
      <div style={{position:"fixed",top:"7%",right:"14%",pointerEvents:"none",zIndex:0,animation:"jellyPulse 5s ease-in-out 0s infinite"}}>
        <ChibiJellyfish/>
      </div>
      {/* Turtle drifts diagonally */}
      <div style={{position:"fixed",top:"35%",pointerEvents:"none",zIndex:0,animation:"driftDiag 30s linear 2s infinite"}}>
        <ChibiTurtle/>
      </div>
      {/* Pufferfish floats right */}
      <div style={{position:"fixed",top:"43%",right:"5%",pointerEvents:"none",zIndex:0,animation:"floatUD 5.5s ease-in-out 1s infinite"}}>
        <ChibiPufferfish/>
      </div>
      {/* Clownfish waves L→R */}
      <div style={{position:"fixed",top:"59%",pointerEvents:"none",zIndex:0,animation:"waveLR 14s ease-in-out 6s infinite"}}>
        <ChibiClownfish/>
      </div>
      {/* Octopus wiggles bottom-left */}
      <div style={{position:"fixed",bottom:"13%",left:"6%",pointerEvents:"none",zIndex:1,animation:"wiggle 3s ease-in-out 0s infinite"}}>
        <ChibiOctopus/>
      </div>
      {/* Crab wiggles bottom-right */}
      <div style={{position:"fixed",bottom:"7%",right:"10%",pointerEvents:"none",zIndex:1,animation:"wiggle 2.5s ease-in-out 0.5s infinite"}}>
        <ChibiCrab/>
      </div>
      {/* Shark crosses R→L */}
      <div style={{position:"fixed",top:"50%",pointerEvents:"none",zIndex:0,animation:"crossRL 24s linear 9s infinite"}}>
        <ChibiShark/>
      </div>
      {/* Manta ray crosses R→L */}
      <div style={{position:"fixed",top:"17%",pointerEvents:"none",zIndex:0,animation:"crossRL 32s linear 14s infinite"}}>
        <ChibiRay/>
      </div>
      {/* Seahorse bobs right */}
      <div style={{position:"fixed",top:"30%",right:"7%",pointerEvents:"none",zIndex:0,animation:"seahorseBob 7s ease-in-out 2s infinite"}}>
        <ChibiSeahorse/>
      </div>
      {/* Starfish spins bottom */}
      <div style={{position:"fixed",bottom:"5%",left:"40%",pointerEvents:"none",zIndex:1,animation:"spinSlow 16s linear 0s infinite"}}>
        <ChibiStarfish/>
      </div>
      {/* Fish school crosses L→R */}
      <div style={{position:"fixed",top:"68%",pointerEvents:"none",zIndex:0,animation:"crossLR 11s linear 3s infinite"}}>
        <ChibiFishSchool/>
      </div>
    </>
  );
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",position:"relative",overflowX:"hidden",background:"linear-gradient(180deg,#0f1f4a 0%,#1a3a6b 12%,#1b6ca8 28%,#2196c4 42%,#5bc4c4 56%,#e8905a 72%,#f0b87a 84%,#f7d4a0 95%,#fcebd4 100%)",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      {[4,91].map((l,i)=>(
        <div key={i} style={{position:"fixed",bottom:0,left:`${l}%`,pointerEvents:"none",zIndex:0,opacity:0.35,animation:`sway ${2.5+i*0.5}s ease-in-out infinite`,transformOrigin:"bottom center"}}>
          {[0,1,2,3].map(j=><div key={j} style={{width:13,height:32,borderRadius:i===0?"50% 0 50% 50%":"0 50% 50% 50%",background:"rgba(50,160,100,0.65)",marginTop:-6}}/>)}
        </div>
      ))}
      ...
      <div style={{position:"relative",zIndex:1,textAlign:"center",padding:"40px 24px 24px",color:"#fff"}}>
        <div style={{position:"absolute",top:80,left:"50%",transform:"translateX(-50%)",width:"130%",height:1,background:"linear-gradient(90deg,transparent,rgba(255,200,100,0.5),rgba(255,140,80,0.35),transparent)",filter:"blur(1px)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",top:60,left:"50%",transform:"translateX(-50%)",width:80,height:40,borderRadius:"50%",background:"radial-gradient(ellipse,rgba(255,200,80,0.25) 0%,transparent 70%)",filter:"blur(8px)",pointerEvents:"none"}}/>
        ...
        <div style={{fontSize:11,color:"rgba(255,220,160,0.75)",letterSpacing:0.5,marginBottom:22}}>6月9日の沖縄まで、一緒に泳ぎ切ろう 🌊</div>
      ...
      <div style={{padding:"14px 13px 80px",position:"relative",zIndex:1}}>
        ...
        {tab==="today"&&(
          <div className="rise">
            ...
            {todayMeals.length>0&&<Glass><SecLabel>🍱 食事記録 ({todayMeals.length}件)</SecLabel>{todayMeals.map(m=><MealRow key={m.id} meal={m} onDelete={()=>deleteMeal(m.id,dk)}/>)}<div style={{display:"flex",justifyContent:"space-between",padding:"8px 10px",borderTop:"1px solid rgba(255,255,255,0.1)",marginTop:4,fontSize:12,fontWeight:700,color:"rgba(255,200,120,0.9)"}}><span>合計</span><span>{Math.round(mTotals.kcal)} kcal</span></div></Glass>}
            {todayEx.length>0&&<Glass><SecLabel>🏃‍♀️ 運動記録 ({todayEx.length}件)</SecLabel>{todayEx.map(e=><ExerciseRow key={e.id} ex={e} onDelete={()=>deleteExercise(e.id,dk)}/>)}<div style={{display:"flex",justifyContent:"space-between",padding:"8px 10px",borderTop:"1px solid rgba(255,255,255,0.1)",marginTop:4,fontSize:12,fontWeight:700,color:"rgba(100,220,180,0.9)"}}><span>運動消費合計</span><span>{Math.round(exBurn)} kcal</span></div></Glass>}
                ...
                <>
                  <div style={{display:"flex",gap:12}}>
                    ...
                    <div style={{flex:1,textAlign:"center",padding:"12px 0",background:"rgba(255,255,255,0.05)",borderRadius:16,border:"1px solid rgba(255,255,255,0.1)"}}>
                      ...
                      <div style={{fontSize:9,color:"rgba(255,255,255,0.25)",marginTop:2}}>身長 159.8cm</div>
                    </div>
                  </div>
                  <div style={{marginTop:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"rgba(255,255,255,0.35)",marginBottom:4}}>
                      <span>低体重 &lt;18.5</span><span>普通 18.5〜25</span><span>肥満 &gt;25</span>
                    </div>
                    <div style={{position:"relative",height:8,borderRadius:99,overflow:"hidden",background:"linear-gradient(90deg,rgba(160,220,255,0.5) 0%,rgba(100,220,180,0.5) 40%,rgba(255,200,100,0.5) 70%,rgba(255,130,100,0.5) 100%)"}}>
                      <div style={{position:"absolute",top:"50%",transform:"translate(-50%,-50%)",left:`${Math.min(Math.max((bmi-14)/(36-14)*100,2),98)}%`,width:12,height:12,borderRadius:"50%",background:"#fff",boxShadow:"0 0 6px rgba(0,0,0,0.4)"}}/>
                    </div>
                  </div>
                </>
              ):<p style={{color:"rgba(255,255,255,0.3)",textAlign:"center",fontSize:13,margin:0}}>記録タブで体重を入力してね 🌊</p>}
            </Glass>
            ...
            <Glass>
              ...
              {CHECKLIST_ITEMS.map(item=>(
                <div key={item.id} onClick={()=>toggleCheck(item.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:12,marginBottom:5,cursor:"pointer",background:checks[item.id]?"rgba(100,220,180,0.12)":"rgba(255,255,255,0.04)",border:`1px solid ${checks[item.id]?"rgba(100,220,180,0.35)":"rgba(255,255,255,0.07)"}`,transition:"all 0.2s"}}>
                  ...
                  <span style={{fontSize:12,fontWeight:600,transition:"all 0.2s",color:checks[item.id]?"rgba(160,255,220,0.9)":"rgba(255,255,255,0.7)",textDecoration:checks[item.id]?"line-through":"none"}}>{item.label}</span>
                </div>
              ))}
            </Glass>
          </div>
        ...
        {tab==="total"&&(
          <div className="rise">
            ...
            <Glass>
              ...
              {[{label:"総摂取",val:Math.round(allTotals.kcal),unit:"kcal",c:"rgba(255,190,90,0.9)"},{label:"総消費（基礎+運動）",val:Math.round(allTotals.burn),unit:"kcal",c:"rgba(100,220,200,0.9)"},{label:"　うち運動消費",val:Math.round(allTotals.exBurn),unit:"kcal",c:"rgba(140,240,200,0.7)"},{label:"総収支",val:Math.round(allTotals.balance),unit:"kcal",c:allTotals.balance<=0?"rgba(100,220,180,0.9)":"rgba(255,130,100,0.9)"},{label:"平均摂取/日",val:avgKcal,unit:"kcal",c:"rgba(255,220,160,0.8)"},{label:"平均消費/日",val:avgBurn,unit:"kcal",c:"rgba(160,255,220,0.8)"}].map(s=>(
                <div key={s.label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
            ...
            <Glass>
              ...
              <div style={{display:"flex",justifyContent:"space-around"}}>
                {[{label:"P タンパク質",val:avgP,goal:120,c1:"#ff9a9e",c2:"#fecfef"},{label:"F 脂質",val:avgF,goal:40,c1:"#ffecd2",c2:"#fcb69f"},{label:"C 炭水化物",val:avgC,goal:150,c1:"#a1ffce",c2:"#60e0e0"}].map(p=>(
                  <div key={p.label} style={{textAlign:"center"}}>
                    <Ring pct={(p.val||0)/p.goal*100} c1={p.c1} c2={p.c2} size={66} stroke={7}>{p.val?Math.round(p.val/p.goal*100):0}%</Ring>
                    <div style={{fontSize:15,fontWeight:900,color:"#fff",marginTop:5}}>{p.val??"-"}<span style={{fontSize:10}}>g</span></div>
                    ...
                    <div style={{overflowX:"auto"}}>
                      <svg width={W} height={H} style={{display:"block",maxWidth:"100%"}}>
                        <defs>
                          ...
                          <linearGradient id="wl" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#a1ffce"/><stop offset="100%" stopColor="#60d0e0"/></linearGradient>
                        </defs>
                        {ticks.map((t,i)=>(
                          <g key={i}>
                            <line x1={PAD.l} x2={W-PAD.r} y1={py(t)} y2={py(t)} stroke="rgba(255,255,255,0.08)" strokeWidth={1} strokeDasharray="3,4"/>
                            <text x={PAD.l-4} y={py(t)+4} textAnchor="end" fontSize={9} fill="rgba(255,255,255,0.35)" fontFamily="'DM Sans',sans-serif">{t}</text>
                          </g>
                        ))}
                        <line x1={PAD.l} x2={W-PAD.r} y1={H-PAD.b} y2={H-PAD.b} stroke="rgba(255,255,255,0.15)" strokeWidth={1}/>
                        <path d={area} fill="url(#wg)"/>
                        <path d={path} fill="none" stroke="url(#wl)" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"/>
                        {pts.map((p,i)=>{
                          const show=weightEntries.length<=7||i===0||i===pts.length-1||i%Math.ceil(pts.length/5)===0;
                          return <g key={i}><circle cx={p.x} cy={p.y} r={3.5} fill="#a1ffce" stroke="rgba(10,40,80,0.7)" strokeWidth={1.5}/>{show&&<text x={p.x} y={H-PAD.b+12} textAnchor="middle" fontSize={8} fill="rgba(255,255,255,0.4)" fontFamily="'DM Sans',sans-serif">{p.d.slice(5)}</text>}</g>;
                        })}
                      </svg>
                    </div>
                  </>
                );
              })():<p style={{color:"rgba(255,255,255,0.3)",textAlign:"center",fontSize:13}}>体重を2日以上記録すると推移グラフが見られます</p>}
            </Glass>
            <Glass>
              <SecLabel>✅ チェックリスト達成率</SecLabel>
              <div style={{textAlign:"center"}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:44,color:"#fff"}}>{allTotals.checkTotal?Math.round(allTotals.checkDone/allTotals.checkTotal*100):0}<span style={{fontSize:20}}>%</span></div>
                <div style={{fontSize:11,color:"rgba(255,200,120,0.7)",fontWeight:600,marginTop:4}}>{allTotals.checkDone} / {allTotals.checkTotal} items</div>
              </div>
              <div style={{height:8,background:"rgba(255,255,255,0.1)",borderRadius:99,marginTop:12,overflow:"hidden"}}>
                <div style={{height:"100%",borderRadius:99,background:"linear-gradient(90deg,#a1ffce,#60e0e0,#fcb69f)",width:`${allTotals.checkTotal?allTotals.checkDone/allTotals.checkTotal*100:0}%`,transition:"width 0.8s"}}/>
              </div>
            </Glass>
          </div>
        )}
          ...
          <div className="rise">
            <Glass>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <input type="date" value={selectedDate} max={dk} onChange={e=>{if(e.target.value)setSelectedDate(e.target.value);}} style={{flex:1,padding:"11px 14px",borderRadius:14,border:"1px solid rgba(255,255,255,0.22)",background:"rgba(255,255,255,0.09)",backdropFilter:"blur(8px)",color:"#fff",fontSize:15,fontFamily:"'DM Sans',sans-serif",fontWeight:600,outline:"none",colorScheme:"dark"}}/>
                {selectedDate!==dk&&<button onClick={()=>setSelectedDate(dk)} style={{padding:"11px 14px",borderRadius:14,border:"1px solid rgba(255,200,100,0.3)",background:"rgba(255,200,100,0.12)",color:"rgba(255,220,150,0.9)",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:12,cursor:"pointer",whiteSpace:"nowrap"}}>今日に戻る</button>}
              </div>
              {selectedDate!==dk&&<div style={{marginTop:8,fontSize:11,color:"rgba(255,180,100,0.7)",fontWeight:600,textAlign:"center"}}>⚠️ 過去の日付を編集中: {selectedDate}</div>}
            </Glass>
            <Glass>
              <SecLabel>🍱 食事を追加</SecLabel>
              <div style={{marginBottom:10}}><label style={lbl}>食事名（任意）</label><input style={inpSm} type="text" placeholder="朝食 / ランチ / おやつ..." value={mealF.name} onChange={e=>setMealF(p=>({...p,name:e.target.value}))}/></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[{key:"kcal",label:"カロリー (kcal)",ph:"500"},{key:"protein",label:"P タンパク質 (g)",ph:"30"},{key:"fat",label:"F 脂質 (g)",ph:"15"},{key:"carb",label:"C 炭水化物 (g)",ph:"60"}].map(fi=>(
                  <div key={fi.key}><label style={lbl}>{fi.label}</label><input style={inpSm} type="number" placeholder={fi.ph} inputMode="decimal" value={mealF[fi.key]} onChange={e=>setMealF(p=>({...p,[fi.key]:e.target.value}))}/></div>
                ))}
              </div>
              <AddBtn flash={flashMeal} label="食事を追加 🍽" onClick={addMeal} bg="linear-gradient(135deg,rgba(255,190,90,0.2),rgba(255,140,80,0.15))"/>
            </Glass>
            <Glass>
              <SecLabel>🏃‍♀️ 運動を追加</SecLabel>
              <div style={{marginBottom:10}}><label style={lbl}>運動名（任意）</label><input style={inpSm} type="text" placeholder="ジム / ウォーキング / 水泳..." value={exF.name} onChange={e=>setExF(p=>({...p,name:e.target.value}))}/></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div><label style={lbl}>消費カロリー (kcal)</label><input style={inpSm} type="number" placeholder="200" inputMode="numeric" value={exF.kcal} onChange={e=>setExF(p=>({...p,kcal:e.target.value}))}/></div>
                <div><label style={lbl}>時間 (分)</label><input style={inpSm} type="number" placeholder="45" inputMode="numeric" value={exF.duration} onChange={e=>setExF(p=>({...p,duration:e.target.value}))}/></div>
              </div>
              <AddBtn flash={flashEx} label="運動を追加 💪" onClick={addExercise} bg="linear-gradient(135deg,rgba(100,220,180,0.2),rgba(96,210,224,0.15))"/>
            </Glass>
            <Glass>
              <SecLabel>⚖️ 体重を記録</SecLabel>
              <label style={lbl}>{selectedDate}の体重 (kg)</label>
              <input style={inp} type="number" placeholder="57.5" inputMode="decimal" step="0.1" value={weightF} onChange={e=>setWeightF(e.target.value)}/>
              <button onClick={saveWeight} style={{width:"100%",padding:"12px",borderRadius:16,marginTop:8,background:flashW?"linear-gradient(135deg,rgba(100,220,180,0.35),rgba(96,210,224,0.35))":"linear-gradient(135deg,rgba(200,160,255,0.2),rgba(160,120,220,0.15))",backdropFilter:"blur(12px)",color:"#fff",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,cursor:"pointer",border:"1px solid rgba(200,160,255,0.3)",boxShadow:"0 4px 16px rgba(0,30,80,0.2)",transition:"all 0.3s"}}>{flashW?"✓ 保存しました！":"体重を保存 ⚖️"}</button>
            </Glass>
            {(()=>{
              const selRec=records[selectedDate]||{},selMeals=selRec.meals||[],selEx=selRec.exercises||[];
              const selMT=sumMeals(selMeals),selEB=sumExercise(selEx);
              if(selMeals.length===0&&selEx.length===0)return null;
              return (
                <Glass>
                  <SecLabel>📋 {selectedDate} の記録まとめ</SecLabel>
                  {selMeals.length>0&&<><div style={{fontSize:11,color:"rgba(255,200,120,0.7)",fontWeight:700,marginBottom:6}}>食事 {selMeals.length}件 · 合計 {Math.round(selMT.kcal)} kcal</div>{selMeals.map(m=><MealRow key={m.id} meal={m} onDelete={()=>deleteMeal(m.id,selectedDate)}/>)}</>}
                  {selEx.length>0&&<><div style={{fontSize:11,color:"rgba(100,220,180,0.7)",fontWeight:700,marginBottom:6,marginTop:selMeals.length>0?8:0}}>運動 {selEx.length}件 · 消費 {Math.round(selEB)} kcal</div>{selEx.map(e=><ExerciseRow key={e.id} ex={e} onDelete={()=>deleteExercise(e.id,selectedDate)}/>)}</>}
                </Glass>
              );
            })()}
          </div>
        ...
        {tab==="edit"&&(
          <div className="rise">
            <Glass style={{marginBottom:14}}>
              ...
              <p style={{fontSize:12,color:"rgba(255,255,255,0.5)",margin:0,lineHeight:1.6}}>タップして展開 → 日ごとの記録を確認・削除できます。食事・運動の個別削除は「記録」タブから行えます。</p>
            </Glass>
            {sortedDates.length===0?(
              <div style={{textAlign:"center",padding:"40px 0",color:"rgba(255,255,255,0.3)",fontWeight:700}}><div style={{fontSize:40,marginBottom:12}}>🌊</div>まだ記録がないよ</div>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,pointerEvents:"none",zIndex:0}}>
        <svg viewBox="0 0 400 70" style={{display:"block",width:"100%",opacity:0.2}}>
          <path d="M20,70 L20,46 Q24,28 28,46 L28,70" fill="#ff8080"/><path d="M28,70 L28,36 Q32,18 36,36 L36,70" fill="#ff8080"/>
          <path d="M360,70 L360,42 Q364,24 368,42 L368,70" fill="#c0a0e0"/><path d="M368,70 L368,52 Q372,36 376,52 L376,70" fill="#c0a0e0"/>
